...
func TestLister_Refresh(t *testing.T) {
	...
	names := l.Names()
	want := []string{"alpha", "beta", "zitcha/frontend"}
...
func TestLister_RepoCache(t *testing.T) {
	...
	resolverExecFunc = func(ctx context.Context, name string, args ...string) ([]byte, error) {
		...
		case name == "sl" && dir != "" && (dir == workspace+"/alpha" || dir == workspace+"/beta"):
			return []byte("https://github.com/zitcha/" + filepathBase(dir) + ".git\n"), nil
		case name == "sl":
	...
	if r, ok := l.Repo("alpha"); !ok || r != "zitcha/alpha" {
		t.Errorf("Repo(alpha) = (%q,%v); want (zitcha/alpha,true)", r, ok)
	}
	if r, ok := l.Repo("beta"); !ok || r != "zitcha/beta" {
		t.Errorf("Repo(beta) = (%q,%v); want (zitcha/beta,true)", r, ok)
	}
