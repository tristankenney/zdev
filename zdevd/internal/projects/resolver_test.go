...
func TestParseGitHubURL(t *testing.T) {
	cases := []struct {
		...
		{"https_trailing_slash", "https://github.com/example/agora/", "example/agora", false, false},
		{"ssh_at_form", "git@github.com:example/agora.git", "example/agora", false, false},
		{"ssh_at_form_no_git", "git@github.com:example/agora", "example/agora", false, false},
		{"ssh_url_form", "ssh://git@github.com/example/agora.git", "example/agora", false, false},
		{"whitespace_trimmed", "  https://github.com/example/agora.git\n", "example/agora", false, false},
		{"empty", "", "", true, false},
		{"gitlab_returns_NGH", "https://gitlab.com/example/agora.git", "", true, true},
		{"bitbucket_returns_NGH", "git@bitbucket.org:example/agora.git", "", true, true},
...
func TestResolveRepo_SaplingSucceeds(t *testing.T) {
	...
	resolverExecFunc = func(ctx context.Context, name string, args ...string) ([]byte, error) {
		...
		case "sl":
			slCalled = true
			return []byte("https://github.com/example/agora.git\n"), nil
		case "git":
			gitCalled = true
	...
	if repo != "example/agora" {
		t.Errorf("repo = %q; want example/agora", repo)
	}
	if !slCalled {
...
func TestResolveRepo_FallsBackToGit(t *testing.T) {
	...
	resolverExecFunc = func(ctx context.Context, name string, args ...string) ([]byte, error) {
		...
		case "git":
			...
			return []byte("git@github.com:example/agora.git\n"), nil
		}
		return nil, errors.New("unexpected exec")
	}
...
func TestResolveRepo_NonGitHubRemote(t *testing.T) {
	...
	resolverExecFunc = func(ctx context.Context, name string, args ...string) ([]byte, error) {
		if name == "sl" {
			return []byte("https://gitlab.com/example/agora.git\n"), nil
		}
