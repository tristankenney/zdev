...
func TestGHProbe_SkipsWhenUnresolved(t *testing.T) {
	cases := []struct {
		...
		{"known_but_empty", ghStubResolver{"zitcha/agora-a": ""}},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
...
// returns "zitcha/agora" for project "zitcha/agora-a", gh is invoked with the
// resolved repo — NOT the raw project name.
func TestGHProbe_UsesResolvedRepoNotProjectName(t *testing.T) {
	var gotArgs []string
	resolver := ghStubResolver{"zitcha/agora-a": "zitcha/agora"}
