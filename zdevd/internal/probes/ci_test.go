...
func TestCIProbe_SkipsWhenUnresolved(t *testing.T) {
	cases := []struct {
		...
		{"unknown_project", stubResolver{}, "zitcha/agora-a"},
		{"known_but_empty", stubResolver{"zitcha/agora-a": ""}, "zitcha/agora-a"},
	}
	for _, c := range cases {
...
// returns "zitcha/agora" for project "zitcha/agora-a", the probe queries
// the resolved repo — NOT the raw project name.
func TestCIProbe_UsesResolvedRepoNotProjectName(t *testing.T) {
	var gotArgs []string
	resolver := stubResolver{"zitcha/agora-a": "zitcha/agora"}
	p := NewCIProbe(func(tmuxctl.Event) {}, ciTestWorkspace(t, "zitcha/agora-a"), resolver)
	p.execFunc = func(ctx context.Context, dir, name string, args ...string) ([]byte, error) {
		gotArgs = append([]string(nil), args...)
		return []byte("[]"), nil
	}
	if err := p.Refresh(context.Background(), "zitcha/agora-a"); err != nil {
		t.Fatalf("Refresh: %v", err)
	}
	...
	if foundRepo != "zitcha/agora" {
		t.Errorf("--repo value = %q; want %q (resolved repo, not raw project name)", foundRepo, "zitcha/agora")
	}
}
