...
func (p *CalendarProbe) Refresh(ctx context.Context, _ string) error {
	...
	return p.rt.Run(ctx, p.Class(), "calendar", calendarProbeTimeout, func(ctx context.Context) error {
		...
		if err != nil {
			p.submit(tmuxctl.CommitmentsRefresh{Source: "ics", FetchErr: fmt.Sprintf("fetch: %v", err), NowNanos: p.now().UnixNano()})
			return fmt.Errorf("calendar fetch: %w", err)
		}
		defer resp.Body.Close()
		if resp.StatusCode != http.StatusOK {
			errMsg := fmt.Sprintf("fetch: unexpected HTTP status %d", resp.StatusCode)
			p.submit(tmuxctl.CommitmentsRefresh{Source: "ics", FetchErr: errMsg, NowNanos: p.now().UnixNano()})
		...
		p.submit(tmuxctl.CommitmentsRefresh{Source: "ics", Commitments: commitments, NowNanos: p.now().UnixNano()})
		return nil
	})
}
