...
// name in slash form ("example/backend") MUST resolve to a tmux session
// whose Name is the dash form ("example-backend") via lookup-time
...
func TestBuildSnapshot_SessionNameNormalization(t *testing.T) {
	...
	snap := drainUntil(t, sub, 300*time.Millisecond, func(s *proto.Snapshot) bool {
		p := findProject(s.Projects, "example/backend")
		return p != nil && p.Status != "absent"
	})
	proj := findProject(snap.Projects, "example/backend")
	if proj == nil {
		t.Fatal("project 'example/backend' not found in snapshot")
	}
...
func TestBuildSnapshot_ProjectListNameStaysCanonical(t *testing.T) {
	...
	proj := findProject(snap.Projects, "example/backend")
	if proj == nil {
		t.Fatal("project 'example/backend' not in snapshot")
	}
	if proj.Name != "example/backend" {
		t.Errorf("Name = %q; want canonical slash form %q", proj.Name, "example/backend")
	}
}

// TestBuildSnapshot_NoDuplicateRows locks the deduplication fix: when a
// project appears in both the project list (slash form, e.g. "example/backend")
// and as a tmux session (dash form, e.g. "example-backend"), the snapshot MUST
...
func TestBuildSnapshot_NoDuplicateRows(t *testing.T) {
	...
	for _, p := range snap.Projects {
		if p.Name == "example/backend" || p.Name == "example-backend" {
			count++
		}
	}
	if count != 1 {
		t.Errorf("found %d rows for example/* project; want exactly 1 (no duplicate dash+slash rows)", count)
	}

	// Also assert the Sessions wire-format field is deduplicated.
	sessionCount := 0
	for _, s := range snap.Sessions {
		if s == "example/backend" || s == "example-backend" {
			sessionCount++
		}
	}
	if sessionCount != 1 {
		t.Errorf("found %d entries in Sessions for example/* project; want exactly 1", sessionCount)
	}
}

// TestSnapWithCurrentSession_SlashProjectHighlight locks the highlight fix:
// when a renderer pane belongs to a tmux session named "example-backend"
// (dash-form), and the project list contains the canonical slash-form name
// "example/backend", the resulting snapshot's CurrentSession MUST be
// "example/backend" (slash-form) so the renderer's p.Name == CurrentSession
// comparison resolves correctly and the project row is highlighted.
//
// Without the fix snapWithCurrentSession sets CurrentSession = "example-backend"
// (raw session name), which never matches p.Name = "example/backend", so no
...
func TestSnapWithCurrentSession_SlashProjectHighlight(t *testing.T) {
	...
	// Tmux session "example-backend" owns pane %42.
	mustSubmit(t, h, tmuxctl.SessionChanged{ID: "$1", Name: "example-backend"})
	mustSubmit(t, h, tmuxctl.WindowAdd{ID: "@1"})
	...
	// highlight works. Dash-form ("example-backend") indicates the bug is present.
	if snap.CurrentSession != "example/backend" {
		t.Errorf("CurrentSession = %q; want %q — slash/dash normalization missing in snapWithCurrentSession",
			snap.CurrentSession, "example/backend")
	}
}
...
func TestSnapWithCurrentSession_WaitAckSuppressesRowMarker(t *testing.T) {
	...
	st.lastVisitTS["example-agora"] = 200 // >= 100
	...
	p := findProject(got.Projects, "example/agora")
	if p == nil {
		t.Fatal("project 'example/agora' missing from snapshot")
	}
	if p.Status != "alive" {
...
func TestSnapWithCurrentSession_UnackedWaitKeepsRowMarker(t *testing.T) {
	...
	st.lastVisitTS["example-agora"] = 100 // < 300
...
func TestSnapWithCurrentSession_StaleAckExpiresAtTierCrossing(t *testing.T) {
	...
	st := newState()
	st.lastVisitTS["example-agora"] = 120 // visited during the warn tier window
