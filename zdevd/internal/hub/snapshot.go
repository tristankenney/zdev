...
func buildSnapshot(st *state, seq int64, sentAt time.Time, now int64) *proto.Snapshot {
	...
	// twins so "zitcha/backend" and "zitcha-backend" never both appear.
	...
	for _, n := range names {
		...
		// "zitcha-backend"). st.projectListNames stores slash-form
		// (e.g., "zitcha/backend"). Normalize to dash-form for all data
