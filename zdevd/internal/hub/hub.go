...
func snapshotStatuses(s *state) map[string]string {
	...
	// "zitcha/backend" and "zitcha-backend" resolve to the same status key.
...
// "zitcha/backend") so that the renderer's p.Name == CurrentSession
// comparison works correctly. Tmux session names are dash-form
// ("zitcha-backend"); we scan projects normalizing p.Name slash→dash to find
