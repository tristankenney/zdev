...
func TestTierCheck_A(t *testing.T) {
	s := stateWithProject("zitcha-agora", projectData{
		WaitStartedTS: 100,
	})
	...
	r := (*recs)[0]
	if r.Project != "zitcha-agora" {
		t.Errorf("Project = %q, want %q", r.Project, "zitcha-agora")
	}
	if r.Msg != "waiting 1m" {
...
func TestTierCheck_H(t *testing.T) {
	...
	s.clientSessions["c1"] = "zitcha-agora"
	recs, fire := makeRecorder()
...
func TestTierCheck_I_FreshAck(t *testing.T) {
	...
	s.lastVisitTS["zitcha-agora"] = 450 // ≥ 100 + 300 (urgent tier floor)
	recs, fire := makeRecorder()
...
func TestTierCheck_I_StaleAckExpires(t *testing.T) {
	...
	s.lastVisitTS["zitcha-agora"] = 150 // post-WaitStartedTS but pre-warn-floor
	recs, fire := makeRecorder()
	...
	r := (*recs)[0]
	if r.Project != "zitcha-agora" {
		t.Errorf("Project = %q, want zitcha-agora", r.Project)
	}
	if r.Msg != "STUCK (15m)" {
		t.Errorf("Msg = %q, want STUCK (15m)", r.Msg)
	}
...
func TestTierCheck_K(t *testing.T) {
	...
	if (*recs)[0].Project != "zitcha-agora" {
		t.Errorf("Project = %q, want dash-form %q", (*recs)[0].Project, "zitcha-agora")
	}
}
