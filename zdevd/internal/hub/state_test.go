...
func TestRecomputeAgents_CapturesOnTransition(t *testing.T) {
	...
	t.Run("A_capture_on_transition_into_waiting", func(t *testing.T) {
		s := buildTestState("zitcha-agora", []string{"%1"}, []string{"claude"})
		...
		recomputeAgents(s, "zitcha-agora")
		...
		if got := s.projectData["zitcha-agora"].WaitContext; got != stubResult {
	...
	t.Run("B_clear_on_transition_out_of_waiting", func(t *testing.T) {
		...
		recomputeAgents(s, "zitcha-agora")
		if s.projectData["zitcha-agora"].WaitContext == "" {
		...
		if got := s.projectData["zitcha-agora"].WaitContext; got != "" {
	...
	t.Run("D_capturer_error_leaves_wait_context_empty", func(t *testing.T) {
		...
		if s.projectData["zitcha-agora"].AgentClaude != "waiting" {
			t.Errorf("AgentClaude = %q; want waiting (recomputeAgents must still complete)", s.projectData["zitcha-agora"].AgentClaude)
		}
	})
	...
	t.Run("E_claude_wins_over_pi_tiebreak", func(t *testing.T) {
		...
		s := buildTestState("zitcha-agora", []string{"%1", "%2"}, []string{"● claude", "● pi"})
	...
	t.Run("G_buildSnapshot_wires_wait_context", func(t *testing.T) {
		s := newState()
		s.paneCapturer = func(paneID string) (string, error) { return "", nil }
		s.sessions["$1"] = &session{ID: "$1", Name: "zitcha-agora", windows: make(map[string]*window)}
		pd := s.projectData["zitcha-agora"]
		pd.WaitContext = "foo\nbar"
		s.projectData["zitcha-agora"] = pd

		snap := buildSnapshot(s, 1, time.Now(), time.Now().Unix())
		proj := findProject(snap.Projects, "zitcha-agora")
		if proj == nil {
			t.Fatal("project 'zitcha-agora' not found in snapshot")
		}
...
func TestApplyEvent_CIRefresh_WritesProjectData(t *testing.T) {
	...
	applyEvent(s, tmuxctl.CIRefresh{
		Project: "zitcha/backend", Status: "completed", Conclusion: "success",
	}, nil)
	pd := s.projectData["zitcha-backend"]
...
func TestApplyEvent_CIRefresh_ClearsStaleData(t *testing.T) {
	...
	s.projectData["zitcha-backend"] = pd

	applyEvent(s, tmuxctl.CIRefresh{
		Project: "zitcha/backend", Status: "", Conclusion: "",
	}, nil)
	got := s.projectData["zitcha-backend"]
...
func TestBuildSnapshot_PropagatesCI(t *testing.T) {
	s := newState()
	s.projectListNames = []string{"zitcha/backend"}
	applyEvent(s, tmuxctl.CIRefresh{
		Project: "zitcha/backend", Status: "completed", Conclusion: "failure",
	}, nil)
	snap := buildSnapshot(s, 1, time.Now(), time.Now().Unix())
