...
func startHubWithNotifier(t *testing.T, debounce time.Duration) (*Hub, *recordingNotifier, func()) {
	...
	h.state.projectData["zitcha-agora"] = projectData{
		AgentClaude:   "waiting",
...
func TestWithNotifier_D(t *testing.T) {
	...
	// Read the state JSON and verify bit0 is set for zitcha-agora.
	...
	v, ok := tierMap["zitcha-agora"]
	if !ok {
		t.Fatal("zitcha-agora missing from waitNotifiedTiers in state.json")
	}
	// JSON numbers decode as float64.
	if v.(float64) != 1 {
		t.Errorf("waitNotifiedTiers[zitcha-agora] = %v, want 1 (bit0 set)", v)
	}
}
