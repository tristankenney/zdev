...
func TestStreamFrames(t *testing.T) {
	...
	// No rail-only breathing line between floor and streams (removed
	// 2026-08-19 \u2014 it read as odd dead space; the stream label alone is
	// enough of a seam).
	for _, l := range lines {
		if l == "  \u2502" {
			t.Errorf("no bare rail line should render between floor and streams:\n%s", out)
		}
	}
...
// The rail-only breathing line between a group's floor and its first stream
// was removed (live feedback, 2026-08-19: it read as odd dead space) — no
// scenario should emit a bare "  │" line any more, whether or not the floor
// painted a visible row before the stream.
func TestStreamFramesNoGapEver(t *testing.T) {
	defer func(m string) { GroupMode = m }(GroupMode)
	GroupMode = "prefix"
	snap := &proto.Snapshot{Projects: []proto.Project{
		{Name: "alpha", Status: "alive"},
	...
	for _, l := range lines {
		if l == "  \u2502" {
			t.Errorf("no bare rail line should ever render between floor and stream:\n%s", out)
		}
	}
}
